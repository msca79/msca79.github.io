...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token'])) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        // Ha HTML fájl és WYSIWYG módból jött a mentés, visszaírjuk a <body> tartalmát
        $saved_file_ext = strtolower(pathinfo($file, PATHINFO_EXTENSION));
        if (in_array($saved_file_ext, ['html', 'htm']) && isset($_POST['wysiwyg']) && $_POST['wysiwyg'] == '1') {
            $original_html = file_get_contents($file_path);
            if (preg_match('/<body[^>]*>/i', $original_html)) {
                $writedata = preg_replace(
                    '/(<body[^>]*>).*?(<\/body>)/is',
                    '$1' . str_replace(['\\', '$'], ['\\\\', '\\$'], $writedata) . '$2',
                    $original_html
                );
            }
        }
...
if (isset($_GET['edit']) && !FM_READONLY) {
    ...
    // WYSIWYG mód HTML fájlokhoz
    $isHtmlFile = in_array(strtolower(pathinfo($file, PATHINFO_EXTENSION)), ['html', 'htm']);
    $isWysiwygMode = $isHtmlFile && $isNormalEditor && (!isset($_GET['env']) || $_GET['env'] == 'wysiwyg');
...
?>
    <div class="path">
                ...
                <div class="btn-group">
                    ...
                    <?php if ($is_text) { ?>
                        <?php if ($isNormalEditor) { ?>
                            ...
                            <?php if ($isHtmlFile) { ?>
                            <a title="WYSIWYG" class="btn btn-sm btn-outline-success" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=wysiwyg"><i class="fa fa-file-text"></i> WYSIWYG</a>
                            <?php } ?>
        ...
        <?php
        if ($is_text && $isWysiwygMode) {
            // Kinyerjük a <body> tartalmát ha van, különben az egészet
            $body_content = $content;
            if (preg_match('/<body[^>]*>(.*?)<\/body>/is', $content, $body_match)) {
                $body_content = $body_match[1];
            }
            echo '<div id="wysiwyg-toolbar-extra" class="mb-2">
                <a class="btn btn-sm btn-outline-primary" href="?p=' . urlencode(trim(FM_PATH)) . '&edit=' . urlencode($file) . '"><i class="fa fa-text-height"></i> Egyszerű szerkesztő</a>
                <a class="btn btn-sm btn-outline-primary" href="?p=' . urlencode(trim(FM_PATH)) . '&edit=' . urlencode($file) . '&env=ace"><i class="fa fa-pencil-square-o"></i> Kód szerkesztő</a>
                <button type="button" class="btn btn-sm btn-success" onclick="tinymce_save()"><i class="fa fa-floppy-o"></i> Mentés</button>
            </div>';
            echo '<textarea id="wysiwyg-editor">' . htmlspecialchars($body_content) . '</textarea>';
            ...
            tinymce.init({
                ...
                toolbar: "undo redo | blocks fontfamily fontsize | bold italic underline strikethrough forecolor backcolor | " +
                         ...
                         "link image table media | code fullscreen | searchreplace",
                images_upload_url: "?p=" + encodeURIComponent("' . FM_PATH . '") + "&upload=1&tinymce=1",
                automatic_uploads: true,
                relative_urls: false,
                content_style: "body { font-family: Arial, sans-serif; font-size: 14px; margin: 1rem; }",
            ...
            function tinymce_save() {
                var content = tinymce.get("wysiwyg-editor").getContent();
                var data = {
                    ajax: true,
                    content: content,
                    wysiwyg: "1",
                ...
                $.ajax({
                    ...
                    success: function() { toast("Sikeresen mentve!"); },
                    error: function(r) { toast("<p style=\'background-color:red\'>" + r.responseText + "</p>"); }
                });
            }
            </script>';
        } elseif ($is_text && $isNormalEditor) {
